...
class SwinTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(int(embed_dim*8), num_classes)
...
def Swin(size_level, num_classes):
    # pdb.set_trace()
    if size_level == 't':
        model = SwinTransformer3D(num_classes = num_classes, patch_norm=True)
    elif size_level == 's':
        model = SwinTransformer3D(depths=[2, 2, 18, 2], num_classes = num_classes)
    elif size_level == 'b':
        model = SwinTransformer3D(embed_dim = 128, depths=[2, 2, 18, 2], num_classes = num_classes)
    elif size_level == 'l':
        model = SwinTransformer3D(embed_dim = 192, depths=[2, 2, 18, 2], num_classes = num_classes)
    return model
